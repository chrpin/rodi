...
DROP SCHEMA IF EXISTS cmt_structured_ci CASCADE;
CREATE SCHEMA cmt_structured_ci;
SET search_path = cmt_structured_ci, pg_catalog;
...
CREATE TABLE person (
    ...
    is_co_author boolean DEFAULT false,
    type integer DEFAULT 0
);
...
COPY co_writepaper (co_author, paper) FROM stdin;
1	947
1	949
3	112
3	113
...
COPY paper (id, paperid, rejectedby, readbymeta_reviewer, hasauthor, title, hasdecision, acceptedby, type) FROM stdin;
...
COPY person (id, name, email, is_user, assignedbyreviewer, is_reviewer, is_author, is_co_author, type) FROM stdin;
...
ALTER TABLE ONLY co_writepaper
...
ALTER TABLE ONLY paper
    ADD CONSTRAINT "Paper_readByMeta-Reviewer_fkey" FOREIGN KEY (readbymeta_reviewer) REFERENCES person(id);
...
ALTER TABLE ONLY co_writepaper
    ADD CONSTRAINT "co-writePaper_Co-author_fkey" FOREIGN KEY (co_author) REFERENCES person(id);
